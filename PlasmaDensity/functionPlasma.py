...
def PhaseShift(Fringe, Background, Img_Single_bk):
    """
    :param Fringe: The fringe X coordinate value with plasma introduce fringe shift.
...
def abel(phi, y, coeff):
    ...
    coeff_ne = 2 * 1.1e21 / coeff_phase / wavelength / coeff
    # coeff_ne = 2* wavelength * nc / pi^2
    # critical density: nc = 1.1e21 [/wavelength^2/cm^3]
    temp = {}
    ...
    f = 1 / np.pi * (np.matmul(np.triu(dphi_mat[0:j, 1::] * temp), ones_v[0:j])) * coeff_ne
    r = r_t * coeff
